...
in "boxes"=[centerbboxX, centerbboxY, bboxwidth, bboxheight] 
where bbox is the rotated box with MinimumBoundingBox

RESULTING FILES TO USE FOR THIS         
...
for e in idlist:
    #print("e=", e)
    reformat.append({"boxes":[],"labels":[],"image_id":e-1,"angle":[],"segmentation":[],"upright boxes":[]})
    #e= e-1
...
for d in range(len(reformat)):
    for c in range(len(data['annotations'])):
        ...
        if data['annotations'][c]["image_id"] == (reformat[d]["image_id"]+1):
            ...
            uboxes=data['annotations'][c]["bbox"]
            ...
            center=bounding_box.rectangle_center
            angle=bounding_box.unit_vector_angle
            height=bounding_box.length_orthogonal
            width=bounding_box.length_parallel
            ...
            #angle=bboxangle(a)
            #print('angle',angle)
            #rotatedangle=bboxangle(rotatedbbox) #old way of calculating angle
            reformat[d]["angle"].append(angle)
            reformat[d]["boxes"].append(box)
            reformat[d]["labels"].append(data['annotations'][c]["category_id"])
            reformat[d]["segmentation"].append(segmentation)
            reformat[d]["upright boxes"].append(uboxes)
...
print('idlist=',idlist) 
data = reformat
with open('REMASTEREDjaccard.json', 'w') as f:
...
'''
path='/data/home/sas44100/g_laufwerk/tutorial/first_implementation/object_detection/data/TEST_7_images(1st).json'
